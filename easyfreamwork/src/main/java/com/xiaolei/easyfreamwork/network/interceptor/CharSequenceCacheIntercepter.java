...
import com.xiaolei.easyfreamwork.network.Catch.CacheManager;
 ...
 */
public class CharSequenceCacheIntercepter implements Interceptor
{
    @Override
            ...
            try
            {
                ...
                if (response.isSuccessful()) // 只有在网络请求返回成功之后，才进行缓存处理，否则，404存进缓存，岂不笑话
                {
                    ...
                    if (responseBody != null)
                    {
                        ...
                        CacheManager.getInstance().setCache(CacheManager.encryptMD5(url + reqBodyStr), responStr);//存缓存，以链接+参数进行MD5编码为KEY存
    ...
    private Response getCacheResponse(Request request, long oldNow)
    {
        ...
        String cacheStr = CacheManager.getInstance().getCache(CacheManager.encryptMD5(url + params));//取缓存，以链接+参数进行MD5编码为KEY取
        if (cacheStr == null)
        {
