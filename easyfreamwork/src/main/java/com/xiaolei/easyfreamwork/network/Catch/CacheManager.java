...
public class CacheManager
{
    ...
    {
        File diskCacheDir = getDiskCacheDir(ApplicationBreage.getInstance().getContext(), CACHE_DIR);
        ...
        if (diskCacheDir.getUsableSpace() > DISK_CACHE_SIZE)
        {
            try
            {
                mDiskLruCache = DiskLruCache.open(diskCacheDir,
                        getAppVersion(ApplicationBreage.getInstance().getContext()), 1/*一个key对应多少个文件*/, DISK_CACHE_SIZE);
